...
import {
  ...
  pressSend,
  setModalState,
  setModalVisibility as setSendLIVEModalVisibility,
  confirmSend,
  setDiscordCode,
...
function* pressSendAsync() {
  ...
  yield all([
    put(setSendLIVEModalVisibility({ isVisible: true })),
...
function* confirmSendAsync() {
  ...
  if (error) {
    if (error.payload.error === 'Missing social proof') {
      yield all([
        put(setSendLIVEModalVisibility({ isVisible: false })),
  ...
  try {
    ...
    if (
      ...
      yield put(
        updateWalletError({
          errorMessage:
            'This wallet has already been associated with an Coliving account.'
        })
      )
      return
    }
