...
import { removeFromContentListLibrary } from 'common/store/contentListLibrary/helpers'
...
export function* repostCollectionAsync(
  ...
  yield* call(
    ...
    collection.content_list_owner_id,
    action.collectionId,
    user
  )
    ...
    contents: [
      {
        ...
        content_list_id: action.smartCollectionName as SmartCollectionVariant
      },
...
export function* saveCollectionAsync(
  ...
  yield* put(
    cacheActions.subscribe(Kind.COLLECTIONS, [
      { uid: subscribedUid, id: collection.content_list_id }
    ])
  )
  ...
  yield* put(
    accountActions.addAccountContentList({
      ...
      name: collection.content_list_name,
...
export function* watchShareCollection() {
  yield* takeEvery(
    ...
    function* (action: ReturnType<typeof socialActions.shareCollection>) {
      ...
      share(link, formatShareText(collection.content_list_name, user.name))
