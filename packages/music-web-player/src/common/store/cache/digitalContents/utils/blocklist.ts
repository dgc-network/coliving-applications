...
const setBlocked = async <T extends DigitalContentMetadata>(digital_content: T) => {
...
export const setDigitalContentsIsBlocked = async <T extends DigitalContentMetadata>(
  digitalContents: T[]
): Promise<T[]> => {
  return await Promise.all(digitalContents.map(setBlocked))
}
