...
import TableDragHandle from 'components/digitalContentsTable/tableDragHandle'
import TableFavoriteButton from 'components/digitalContentsTable/tableFavoriteButton'
...
import TableRepostButton from 'components/digitalContentsTable/tableRepostButton'
import UserBadges from 'components/userBadges/userBadges'
import { fullDigitalContentPage } from 'utils/route'

import styles from './DigitalContentsTable.module.css'
...
const digitalContentNameCell = (val, record, props) => {
  const deleted = record.is_delete || record.user?.is_deactivated
  return (
    <div
      ...
      onClick={(e) => {
        e.stopPropagation()
        if (!deleted) props.onClickDigitalContentName(record)
      }}
    >
      <div className={cn(styles.textCell, { [styles.digitalContentName]: !deleted })}>
        {val}
...
const DraggableRow = (props) => {
  ...
  if (children.length > 0) {
    ...
    const link = record.user ? fullDigitalContentPage(record.permalink) : ''
    return (
      <Draggable
        elementType='tr'
        key={dataRowKey}
...
const columns = {
  ...
  colDigitalContentName: { name: 'colDigitalContentName', minWidth: 200 },
...
class DigitalContentsTable extends Component {
  state = {
  ...
  checkDropColumn = () => {
    const table = document.querySelector(
      `.${styles.digitalContentsTableContainer} table`
    )
    ...
    const columns =
      ...
      [
        ...
        },
        {
          ...
          render: (val, record) =>
            loading ? <Loading /> : digitalContentNameCell(val, record, this.props)
        },
        {
          title: <Tooltip text='Author'>{'Author'}</Tooltip>,
          dataIndex: 'author',
          key: 'author',
          className: 'colLandlordName',
          sorter: loading
            ? null
            : (a, b) => alphaSortFn(a.author, b.author, a.key, b.key),
          render: (val, record) =>
  ...
  onToggleShowDigitalContents = () => {
    const limit =
      this.state.limit === this.props.limit
        ? this.props.dataSource.length
        : this.props.limit
    this.setState({ limit })
    this.props.didToggleShowDigitalContents()
  }
  ...
  onTableChange = (pagination, filters, sorters, extras) => {
    this.props.onSortDigitalContents(sorters)
  }
  ...
  onDragEnd = (result) => {
    ...
    if (
      ...
      return
    this.props.onReorderDigitalContents(source.index, destination.index)
  }
  ...
  render() {
    ...
    return (
      <div
        className={cn(styles.digitalContentsTableContainer, {
      ...
      >
        <DragDropContext onDragEnd={this.onDragEnd}>
          ...
          {limit && dataSource.length > limit ? (
            <ShowLimitTab
              onClick={this.onToggleShowDigitalContents}
...
DigitalContentsTable.propTypes = {
  ...
  onSortDigitalContents: PropTypes.func,
  onReorderDigitalContents: PropTypes.func,
  ...
  didToggleShowDigitalContents: PropTypes.func,
  animateTransitions: PropTypes.bool
}

DigitalContentsTable.defaultProps = {
  loading: false,
  loadingRowsCount: 10,
  dataSource: [],
  onClickRow: () => {},
...
export default DigitalContentsTable
