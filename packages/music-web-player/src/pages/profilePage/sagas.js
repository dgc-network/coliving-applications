...
import OpenSeaClient from 'services/openseaClient/openSeaClient'
...
import SolanaClient from 'services/solanaClient/solanaClient'
...
function* fetchProfileAsync(action) {
  try {
    ...
    const isSubscribed = yield call(
      ColivingBackend.getUserSubscribed,
      user.user_id
    )
    yield put(
...
function* fetchFolloweeFollows(action) {
  ...
  const followeeFollows = yield call(
    ColivingBackend.getFolloweeFollows,
    profileUserId,
    action.limit,
    action.offset
  )
...
export function* updateProfileAsync(action) {
  ...
  if (cid) {
    try {
      ...
      const content_list_library = metadata.content_list_library
      metadata = merge(metadataFromIPFS, metadata)
      metadata.collectibles = collectibles
      metadata.content_list_library = content_list_library
    } catch (e) {
  ...
  yield put(
    confirmerActions.requestConfirmation(
      ...
      function* (confirmedUser) {
        ...
        yield setColivingAccountUser(confirmedUser)
  ...
  yield takeEvery(
    ...
    function* (action) {
      if (action.update) {
        try {
          yield call(
            ColivingBackend.updateUserSubscription,
            action.userId,
            action.isSubscribed
          )
        } catch (err) {
